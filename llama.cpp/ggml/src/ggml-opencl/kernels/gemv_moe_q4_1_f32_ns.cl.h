...
R"(static inline float8 q4_1_to_fp32_packed8(ushort2 q4x8, half s, half m) {
)"
R"(    float8 fp32x8;
)"
R"(    fp32x8.s0 = (float)((q4x8.s0 & 0x000F) * s + m);
)"
R"(    fp32x8.s1 = (float)(((q4x8.s0 & 0x00F0) >> 4) * s + m);
)"
R"(    fp32x8.s2 = (float)(((q4x8.s0 & 0x0F00) >> 8) * s + m);
)"
R"(    fp32x8.s3 = (float)(((q4x8.s0 & 0xF000) >> 12) * s + m);
)"
R"(    fp32x8.s4 = (float)((q4x8.s1 & 0x000F) * s + m);
)"
R"(    fp32x8.s5 = (float)(((q4x8.s1 & 0x00F0) >> 4) * s + m);
)"
R"(    fp32x8.s6 = (float)(((q4x8.s1 & 0x0F00) >> 8) * s + m);
)"
R"(    fp32x8.s7 = (float)(((q4x8.s1 & 0xF000) >> 12) * s + m);
)"
R"(    return fp32x8;
)"
R"(}
...
R"(__kernel void kernel_gemv_moe_q4_1_f32_ns(
)"
...
R"(    for (uint ib00 = sgid; ib00 < (ne00 / QK_Q4_1); ib00 += N_SIMDGROUP) {
)"
R"(
)"
...
R"(        half regM = src0_m[ib00 * ne01 + i01 + expert_offset];
)"
R"(        half regS = src0_d[ib00 * ne01 + i01 + expert_offset];
)"
R"(
)"
R"(        float8 fp32x8 = q4_1_to_fp32_packed8(as_ushort2(regQ.s0), regS, regM);
)"
R"(
)"
R"(        float4 shared_y4;
...
R"(        fp32x8 = q4_1_to_fp32_packed8(as_ushort2(regQ.s1), regS, regM);
)"
R"(
)"
...
R"(        fp32x8 = q4_1_to_fp32_packed8(as_ushort2(regQ.s2), regS, regM);
)"
R"(
)"
...
R"(        fp32x8 = q4_1_to_fp32_packed8(as_ushort2(regQ.s3), regS, regM);
)"
R"(
)"
