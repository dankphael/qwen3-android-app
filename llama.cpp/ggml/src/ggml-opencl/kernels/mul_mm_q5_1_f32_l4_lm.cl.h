...
R"(kernel void kernel_mul_mm_q5_1_f32_l4_lm(
)"
...
R"(                global uchar4 * qs = src0_qs + ib*4 + iqs;
)"
R"(                uchar4 q = *qs;
)"
R"(
...
R"(                float4 v1 = convert_float4((q & (uchar)0x0F) | (b_lo << (uchar)4)) * d + m;
)"
R"(                float4 v2 = convert_float4((q >> (uchar)4) | (b_hi << (uchar)4)) * d + m;
)"
R"(
)"
