...
R"(kernel void kernel_mul_mm_q5_0_f32_l4_lm(
)"
R"(    global uchar4 * src0_qs,
)"
R"(    global uint   * src0_qh,
)"
...
R"(                uint qh_val = src0_qh[ib];
)"
R"(
)"
R"(                global uchar4 * qs_ptr = src0_qs + ib*4 + iqs;
)"
R"(                uchar4 q = *qs_ptr;
)"
R"(
)"
R"(                uint qh_lo = qh_val >> (iqs * 4);
)"
R"(                uint qh_hi = qh_val >> (iqs * 4 + 16);
)"
R"(
)"
R"(                uchar4 b_lo = (uchar4)((uchar)qh_lo, (uchar)(qh_lo >> 1), (uchar)(qh_lo >> 2), (uchar)(qh_lo >> 3)) & (uchar)1;
)"
R"(                uchar4 b_hi = (uchar4)((uchar)qh_hi, (uchar)(qh_hi >> 1), (uchar)(qh_hi >> 2), (uchar)(qh_hi >> 3)) & (uchar)1;
)"
R"(
)"
R"(                float4 v1 = (convert_float4((q & (uchar)0x0F) | (b_lo << (uchar)4)) - 16.0f) * d;
)"
R"(                float4 v2 = (convert_float4((q >> (uchar)4) | (b_hi << (uchar)4)) - 16.0f) * d;
)"
R"(
)"
