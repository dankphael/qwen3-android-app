...
R"(#define dequantize_q4_1(q4, a_f16, scale, m) \
)"
R"(    a_f16.s0 = (half)(q4.s0 & 0x000F) * scale + m; \
)"
R"(    a_f16.s1 = (half)((q4.s0 & 0x00F0) >> 4) * scale + m; \
)"
R"(    a_f16.s2 = (half)((q4.s0 & 0x0F00) >> 8) * scale + m; \
)"
R"(    a_f16.s3 = (half)((q4.s0 & 0xF000) >> 12) * scale + m; \
)"
R"(    a_f16.s4 = (half)(q4.s1 & 0x000F) * scale + m; \
)"
R"(    a_f16.s5 = (half)((q4.s1 & 0x00F0) >> 4) * scale + m; \
)"
R"(    a_f16.s6 = (half)((q4.s1 & 0x0F00) >> 8) * scale + m; \
)"
R"(    a_f16.s7 = (half)((q4.s1 & 0xF000) >> 12) * scale + m; \
)"
R"(    a_f16.s8 = (half)(q4.s2 & 0x000F) * scale + m; \
)"
R"(    a_f16.s9 = (half)((q4.s2 & 0x00F0) >> 4) * scale + m; \
)"
R"(    a_f16.sa = (half)((q4.s2 & 0x0F00) >> 8) * scale + m; \
)"
R"(    a_f16.sb = (half)((q4.s2 & 0xF000) >> 12) * scale + m; \
)"
R"(    a_f16.sc = (half)(q4.s3 & 0x000F) * scale + m; \
)"
R"(    a_f16.sd = (half)((q4.s3 & 0x00F0) >> 4) * scale + m; \
)"
R"(    a_f16.se = (half)((q4.s3 & 0x0F00) >> 8) * scale + m; \
)"
R"(    a_f16.sf = (half)((q4.s3 & 0xF000) >> 12) * scale + m; \
)"
R"(
)"
R"(
...
R"(kernel void kernel_gemm_moe_q4_1_f32_ns(
)"
...
R"(        // Load scale and m for current Q4_1 block
)"
R"(        uint sm_offset = s_sub_offset + get_global_id(0);
)"
R"(        half s = src0_d[sm_offset];
)"
R"(        half m = src0_m[sm_offset];
)"
R"(
)"
...
R"(        dequantize_q4_1(as_ushort4(q4x16), reg_a, s, m);
)"
R"(
)"
...
R"(        // Load next 16 q (64-bits) in transposed layout
)"
R"(        q4x16.x = read_imageui(src0_q, q_sub_offset + sub_block_id_m).x;
)"
R"(        q4x16.y = read_imageui(src0_q, q_sub_offset + sub_block_id_m + ne01).x;
