...
R"(inline float block_q5_1_dot_y_flat(
)"
R"(    global const uchar * x,
)"
R"(    global const uint  * qh_ptr,
)"
...
R"(    global const ushort * qs = ((global const ushort *)(x + il));
)"
R"(
)"
...
R"(    uint qh_val = *qh_ptr;
)"
R"(    uchar qh_lo = (uchar)((qh_val >> il) & 0xFF);
)"
R"(    uchar qh_hi = (uchar)((qh_val >> (il + 16)) & 0xFF);
)"
R"(
)"
R"(    float qh_sum = 0.0f;
)"
R"(    qh_sum += yb[0]  * (float)((qh_lo >> 0) & 1);
)"
R"(    qh_sum += yb[1]  * (float)((qh_lo >> 1) & 1);
)"
R"(    qh_sum += yb[2]  * (float)((qh_lo >> 2) & 1);
)"
R"(    qh_sum += yb[3]  * (float)((qh_lo >> 3) & 1);
)"
R"(    qh_sum += yb[4]  * (float)((qh_lo >> 4) & 1);
)"
R"(    qh_sum += yb[5]  * (float)((qh_lo >> 5) & 1);
)"
R"(    qh_sum += yb[6]  * (float)((qh_lo >> 6) & 1);
)"
R"(    qh_sum += yb[7]  * (float)((qh_lo >> 7) & 1);
)"
R"(    qh_sum += yb[16] * (float)((qh_hi >> 0) & 1);
)"
R"(    qh_sum += yb[17] * (float)((qh_hi >> 1) & 1);
)"
R"(    qh_sum += yb[18] * (float)((qh_hi >> 2) & 1);
)"
R"(    qh_sum += yb[19] * (float)((qh_hi >> 3) & 1);
)"
R"(    qh_sum += yb[20] * (float)((qh_hi >> 4) & 1);
)"
R"(    qh_sum += yb[21] * (float)((qh_hi >> 5) & 1);
)"
R"(    qh_sum += yb[22] * (float)((qh_hi >> 6) & 1);
)"
R"(    qh_sum += yb[23] * (float)((qh_hi >> 7) & 1);
)"
R"(
)"
R"(    return d * (acc.s0 + acc.s1 + acc.s2 + acc.s3 + 16.0f * qh_sum) + sumy * m;
)"
R"(}
)"
R"(
...
R"(        global void * src0_qs,
)"
R"(        global void * src0_qh,
)"
...
R"(    ulong offset0_qs = offset0 * (QK5_1/2);
)"
R"(
)"
R"(    global uchar * x  = (global uchar *) src0_qs + offset0_qs;
)"
R"(    global uint  * qh = (global uint  *) src0_qh + offset0;
)"
R"(    global half  * d  = (global half  *) src0_d  + offset0;
)"
R"(    global half  * ms = (global half  *) src0_m  + offset0;
)"
...
R"(    global float * yb = y + ix * QK5_1 + il;
)"
R"(
)"
...
R"(        sumf.s0 += block_q5_1_dot_y_flat(x + ib*(QK5_1/2) + 0*nb*(QK5_1/2), qh + ib + 0*nb, d + ib + 0*nb, ms + ib + 0*nb, sumy, yl, il, yb);
)"
R"(        sumf.s1 += block_q5_1_dot_y_flat(x + ib*(QK5_1/2) + 1*nb*(QK5_1/2), qh + ib + 1*nb, d + ib + 1*nb, ms + ib + 1*nb, sumy, yl, il, yb);
)"
R"(        sumf.s2 += block_q5_1_dot_y_flat(x + ib*(QK5_1/2) + 2*nb*(QK5_1/2), qh + ib + 2*nb, d + ib + 2*nb, ms + ib + 2*nb, sumy, yl, il, yb);
)"
R"(        sumf.s3 += block_q5_1_dot_y_flat(x + ib*(QK5_1/2) + 3*nb*(QK5_1/2), qh + ib + 3*nb, d + ib + 3*nb, ms + ib + 3*nb, sumy, yl, il, yb);
)"
R"(
)"
R"(        yb += QK5_1 * (N_SIMDWIDTH/2);
)"
R"(    }
)"
R"(
...
R"(kernel void kernel_mul_mv_q5_1_f32_flat(
)"
R"(        global void * src0_qs,
)"
R"(        global void * src0_qh,
...
R"(    mul_vec_q_n_f32_flat(src0_qs, src0_qh, src0_d, src0_m, src1, dst, ne00, ne01, ne02, ne10, ne12, ne0, ne1, r2, r3);
)"
R"(}
)"
