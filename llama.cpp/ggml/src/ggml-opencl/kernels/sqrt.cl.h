...
R"(kernel void kernel_sqrt_cont_f32(
)"
R"(    global float * src0,
)"
R"(    ulong          offset0,
...
R"(    dst[gid] = sqrt(src0[gid]);
)"
R"(}
)"
R"(
)"
R"(kernel void kernel_sqrt_cont_f32_4(
)"
...
R"(kernel void kernel_sqrt_cont_f16(
)"
R"(    global half * src0,
)"
R"(    ulong         offset0,
...
R"(    dst[gid] = convert_half(sqrt(convert_float(src0[gid])));
)"
R"(}
)"
R"(
)"
R"(kernel void kernel_sqrt_cont_f16_4(
)"
R"(    global half4 * src0,
)"
R"(    ulong          offset0,
...
R"(    dst[gid] = convert_half4(sqrt(convert_float4(src0[gid])));
)"
R"(}
)"
