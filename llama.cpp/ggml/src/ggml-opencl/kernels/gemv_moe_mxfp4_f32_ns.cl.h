...
R"(__kernel void kernel_gemv_moe_mxfp4_f32_ns(
)"
...
R"(        float4 acc = shared_y4 * convert_float4(fp16x8.lo);
)"
R"(
)"
R"(        shared_y4 = read_imagef(src1, (offset + 1));
)"
R"(        acc += shared_y4 * convert_float4(fp16x8.hi);
)"
R"(
)"
...
R"(        acc += shared_y4 * convert_float4(fp16x8.lo);
)"
R"(
)"
