...
R"(#define T_FLOAT half
)"
R"(#define T_FLOAT4 half4
)"
R"(#define VSTORE_T_FLOAT4(data, offset, p) vstore_half4_rte(data, offset, p)
)"
R"(#else
)"
R"(#define T_FLOAT float
)"
R"(#define T_FLOAT4 float4
)"
R"(#define VSTORE_T_FLOAT4(data, offset, p) vstore4(data, offset, p)
)"
R"(#endif
)"
R"(
...
R"(    global T_FLOAT* knl_data = (global T_FLOAT*) ((global char*)p_knl + off_knl);
)"
R"(    global T_FLOAT* src_data = (global T_FLOAT*) ((global char*)p_src + off_src);
)"
R"(    global T_FLOAT* dst_data = (global T_FLOAT*) ((global char*)p_dst + off_dst);
)"
R"(
)"
R"(    const uint K = Cout;
...
R"(    local T_FLOAT* Ash = (local T_FLOAT*)shared;
)"
R"(    local T_FLOAT4* Bsh = (local T_FLOAT4*) &Ash[BS_K * BS_CRS];
)"
R"(
)"
...
R"(                Ash[k_l * BS_CRS + crs_l] = (T_FLOAT)0.0f;
)"
R"(            }
)"
R"(        }
...
R"(                            ((T_FLOAT*)&val)[v] = src_data[src_idx];
)"
R"(                        }
)"
R"(                    }
...
R"(                T_FLOAT4 regB = Bsh[crs_l * BS_NPQ_VEC + lid_npq * TS_NPQ_VEC + npq_l_vec_reg];
)"
R"(                for (uint k_l_reg = 0; k_l_reg < TS_K; ++k_l_reg) {
)"
R"(                    regC[k_l_reg][npq_l_vec_reg] = mad(convert_float(regA[k_l_reg]), convert_float4(regB), regC[k_l_reg][npq_l_vec_reg]);
)"
R"(                }
)"
R"(            }
...
R"(                VSTORE_T_FLOAT4(regC[k_l_reg][npq_l_vec_reg], 0, &dst_data[dst_idx]);
)"
R"(            } else {
)"
...
R"(                        dst_data[dst_idx_s] = (T_FLOAT)(((float*)&res)[v]);
)"
R"(                    }
)"
R"(                }
