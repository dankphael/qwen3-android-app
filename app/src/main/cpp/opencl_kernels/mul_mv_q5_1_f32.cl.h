...
R"(inline float block_q5_1_dot_y(
)"
R"(    global const struct block_q5_1 * qb_curr,
)"
R"(    float sumy,
)"
R"(    float16 yl,
...
R"(    global const ushort * qs = ((global const ushort *)((global const uchar *) qb_curr + 8 + il));
)"
R"(
)"
...
R"(    uint qh_val = *((global const uint *)((global const uchar *) qb_curr + 4));
)"
...
R"(    global struct block_q5_1 * x = (global struct block_q5_1 *) src0 + offset0;
)"
...
R"(        sumf.s0 += block_q5_1_dot_y(x+ib+0*nb, sumy, yl, il, yb);
)"
R"(        sumf.s1 += block_q5_1_dot_y(x+ib+1*nb, sumy, yl, il, yb);
)"
R"(        sumf.s2 += block_q5_1_dot_y(x+ib+2*nb, sumy, yl, il, yb);
)"
R"(        sumf.s3 += block_q5_1_dot_y(x+ib+3*nb, sumy, yl, il, yb);
)"
R"(
)"
...
R"(kernel void kernel_mul_mv_q5_1_f32(
)"
R"(        global void * src0,
)"
R"(        ulong offset0,
