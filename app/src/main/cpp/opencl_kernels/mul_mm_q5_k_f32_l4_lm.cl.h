...
R"(kernel void kernel_mul_mm_q5_k_f32_l4_lm(
)"
...
R"(                int qsi = n * 32 + (iqs % 16) * 2;
)"
R"(
)"
R"(                global uchar * scales = src0_s + ib * 12;
)"
R"(
)"
R"(                int scidx0      = (is < 4) ? is     : (is + 4);
)"
R"(                int scidx1      = (is < 4) ? is     : (is - 4);
)"
R"(                int scidxmask1  = (is < 4) ? 0x30   : 0xC0;
)"
R"(                int scidxshift1 = (is < 4) ? 0      : 2;
)"
R"(                int mbidx0      = is + 4;
)"
R"(                int mbidx1      = (is < 4) ? is + 4 : is;
)"
R"(                int mbidxmask0  = (is < 4) ? 0xF    : 0xF0;
)"
R"(                int mbidxshift0 = (is < 4) ? 0      : 4;
)"
R"(                int mbidxmask1  = (is < 4) ? 0x30   : 0xC0;
)"
R"(                int mbidxshift1 = (is < 4) ? 0      : 2;
)"
R"(
)"
R"(                uchar sc    = (scales[scidx0] & 0xF) | ((scales[scidx1] & scidxmask1) >> scidxshift1);
)"
R"(                uchar mbyte = ((scales[mbidx0] & mbidxmask0) >> mbidxshift0) | ((scales[mbidx1] & mbidxmask1) >> mbidxshift1);
)"
R"(
)"
R"(                float d =  (float)src0_d[ib]  * (float)sc;
)"
R"(                float m = -(float)src0_dm[ib] * (float)mbyte;
)"
R"(
)"
R"(                int qh_base = (iqs % 16) * 2;
)"
R"(                int bit_pos  = 2*n + b;
)"
R"(                uchar h0 = (src0_qh[ib*32 + qh_base + 0] >> bit_pos) & 1;
)"
R"(                uchar h1 = (src0_qh[ib*32 + qh_base + 1] >> bit_pos) & 1;
)"
R"(                uchar h2 = (src0_qh[ib*32 + qh_base + 2] >> bit_pos) & 1;
)"
R"(                uchar h3 = (src0_qh[ib*32 + qh_base + 3] >> bit_pos) & 1;
)"
R"(
)"
R"(                global uchar4 * qs = src0_q + ib*32 + (qsi >> 2);
)"
R"(                uchar4 q = *qs;
)"
R"(                float4 v1 = (convert_float4((uchar4)(
)"
R"(                    ((q.s0 >> (b * 4))&0x0F) | (h0 << 4),
)"
R"(                    ((q.s1 >> (b * 4))&0x0F) | (h1 << 4),
)"
R"(                    ((q.s2 >> (b * 4))&0x0F) | (h2 << 4),
)"
R"(                    ((q.s3 >> (b * 4))&0x0F) | (h3 << 4)
)"
R"(                )))*d + m;
)"
R"(
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 0) * BM + loadc_a + l] = v1.s0;
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 1) * BM + loadc_a + l] = v1.s1;
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 2) * BM + loadc_a + l] = v1.s2;
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 3) * BM + loadc_a + l] = v1.s3;
)"
R"(            } else {
)"
